...
#define AZURE_PNP_MODEL_ID "dtmi:azureiot:devkit:freertos:WeatherStation;1"
...
int azure_pnp_send_telemetry(azure_iot_t* azure_iot)
{
      ...
      || difftime(now, last_telemetry_send_time) >= telemetry_frequency_in_seconds)
  {
    ...
    // Sending telemetry data to IOt Central
...
void configureBME(){
 ...
 // while (!Serial);
  ...
  if (!bme.begin()) {
     LogInfo("Could not find a valid BME680 sensor, check wiring!");
    while (1);
  }
    ...
    size_t* payload_buffer_length)
{
  ...
  if (! bme.performReading()) {
     LogInfo("Failed to perform reading :(");
    return RESULT_OK;
  }

  LogInfo("BME temp: %f",bme.temperature);
  LogInfo("BME humid: %f %s",bme.humidity,"%");
